...
> **Note on accuracy:** the percentage comes straight from Grok Build's own billing check, so it should match what the CLI shows you. That said, `creditUsagePercent` itself is only sent once xAI has something worth reporting — per real log evidence, it's omitted (never sent as `0`) until usage crosses roughly 1% for the period, and then stays present for the rest of that period. Whenever a billing period is known but the field is still omitted, the plugin shows an empty ring + `0%` (i.e. "under ~1%, not yet reported"), which needed a real-world log to get right — see [issue #3](https://github.com/ENjxzlt/Grok-Usage-Streamcontroller/issues/3) if you're curious about the evidence. The optional last-turn cost figure is a separate guess: `costUsdTicks`' unit isn't documented, so this assumes 1,000,000,000 ticks = $1 (inferred from the sample value in the originating issue) — treat it as approximate.
